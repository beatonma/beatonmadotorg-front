...
const compressCommits = commits => groupCommitsByRepo(commits);
  ...
  const uniqueMessages = props.changes
    ...
    .map(change => {
      if (change.message.length > MESSAGE_CHAR_LIMIT) {
        change.message = change.message.slice(0, MESSAGE_CHAR_LIMIT) + "…";
      }
      return change;
    });
  ...
  return (
    <span className="github-recent-repo-meta">
      <CommitCount
        ...
        timestamp={formatTimestampString(props.commit.start, props.commit.end)}
      />
    </span>
  );
}
